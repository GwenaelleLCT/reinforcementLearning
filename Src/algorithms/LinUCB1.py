...
class LinUCB1():
    ...
    def choose_action(self):
        ...
        if np.min(self.arms_payoff_vectors["tries"]) == 0 : # If there is at least one arm that has not been tried yet, we choose it
            i=0
        ...
        if arm_chosen_index == -1 : 
            ...
            for arm in self.arms_pool['arm_id']:
                ...
                expected_payoff[i] = self.arms_payoff_vectors["cumulated_rewards"][arm_pos] / \
                                        ...
                                        sqrt(2*log(total_tries)/self.arms_payoff_vectors["tries"][arm_pos])
